...
For example, "`Log &lsqb;handshake]debug &lsqb;~net,~mm]info notice stdout`"
...
**HiddenServicePort** _VIRTPORT_ &lsqb; _TARGET_ ]
...
specify an IPv6 target as  &lsqb;addr]:port. Unix paths may be quoted, and may
use standard C escapes.) You may also have multiple lines with the same
VIRTPORT: when a user connects to that VIRTPORT, one of the TARGETs from those
...
Roger Dingledine &lsqb;arma at mit.edu], Nick Mathewson &lsqb;nickm at
alum.mit.edu].
